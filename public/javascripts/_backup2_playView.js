		...
		*/
		...
		//shadow
		{
				...
				switch(thisNote.m_type) {
							...
							if(thisNote.status == "Untouched") {
								if (touchTime < thisTime - judge.g) {
									...
									for (var [id, f] of touchHold) {
										if (isValidTouch(f, 0)  && between(f.x, mtox(thisNote.m_position, 0) - mtow(thisNote.m_width, 0)/2 - expand, mtox(thisNote.m_position, 0) + mtow(thisNote.m_width, 0)/2 + expand)) {
					...
					case "HOLD":
						...
						if (isMobile) {
							...
							/*else if(missHoldD.has(i)) {
								var rdis = hiSpeed * (missHoldD.get(i) - thisTime);
								if (rdis < -ud/2-zip) {
									missHoldD.delete(i);
								}
								else {
									noteHoldShow.push([0, width, dis2 - rdis, x, rdis, extra, 0, false, 1.0]);
								}
							}*/
			...
			for(var i = 0; i < noteLeft.length; ++i) {
				thisNote = noteLeft[i];
				if(!thisNote || (isMobile && thisNote && thisNote.status != "Untouched" && !(thisNote.status == "Miss" && ! missHoldFindL[i]) && thisNote.m_type != "HOLD")) continue;
				...
				switch(thisNote.m_type) {
					case "NORMAL":
						...
						else if(!noteLeftHit[i]) {
							if(touchTime <= thisTime) {
								noteLeftHit[i] = true;
								hitAnime(thisNote, 1, "Perfect");
							}
						}
						if(((! isMobile && dis >= 0) || isMobile) && dis <= windowWidth/2 - lr) {
							if (dis < -hideHeight - (lr - hideHeight)/hide) {
								missHoldFindL[i] = true;
								break;
							}
							var ndis = dis;
							if(deeMode && leftJump) {
					...
					case "CHAIN":
						if (isMobile) {
							if(thisNote.status == "Untouched") {
								if (touchTime < thisTime - judge.g) {
									...
									for (var [id, f] of touchHold) {
										if (isValidTouch(f, 1) && between(f.y, mtox(thisNote.m_position, 1) - mtow(thisNote.m_width, 1)/2 - expand, mtox(thisNote.m_position, 1) + mtow(thisNote.m_width, 1)/2 + expand)) {
											if (Math.abs(thisNote.m_time*(60/barpm) - thisTime) <= judge.p) {
												hitAnime(thisNote, 1, "Perfect");
											}
											else if (Math.abs(thisNote.m_time*(60/barpm) - thisTime) <= judge.gr) {
						...
						if(((! isMobile && dis >= 0) || isMobile) && dis <= windowWidth - lr) {
							var ndis = dis;
							if(deeMode && leftJump) {
								dis = Math.cos(dis / (windowWidth / 2 - lr) * Math.PI / 2) * dis * lrJumpHeight;
							}
					...
					case "HOLD":
						...
						if (isMobile) {
							...
							var miss = missHoldFindL[i];
							if (miss) {
								if (miss.half) {
									if (thisTime - miss.missTime > 0.3) {
										missHoldFindL[i] = false;
										missHoldL.delete(i);
										break;
									}
									else {
										extra = 180 - Math.round(0) % 180;
										noteHoldShow.push([1, width, dis2, x, 0, extra, 0, false, 1 - (thisTime - miss.missTime)/0.3]);
										break;
									}
								}
								else {
									if (dis < -hideHeight - (lr - hideHeight)/hide) {
										...
										extra = 180 - Math.round(-dis) % 180;
										noteHoldShow.push([1, width, dis2 - dis, x, dis, extra, 0, false, 1.0]);
										break;
									}
								}
							}
						...
						if (isMobile) {
							if (holdCheckL.has(i)) {
								...
								if (holding) {
									...
									if (holdCheckL.get(i) + 0 >= noteLeft[thisNote.m_subId].m_time*(60/barpm) - judge.p) {
										holdCheckL.delete(i);
										hitAnime(noteLeft[thisNote.m_subId], 1, "Perfect");
									}
									else {
										noteHoldShow.push([1, width, dis2 - dis, x, dis, extra, 0, true, 1.0]);
										break;
									}
								}
								else {
									extra = 180 - Math.round(-dis) % 180;
									dis = 0;
									var releaseTime = holdCheckL.get(i);
									if (thisTime - releaseTime > holdLeaveTime) {
										if (Math.abs(releaseTime - subTime) <= judge.p) {
											hitAnime(noteLeft[thisNote.m_subId], 1, "Perfect");
										}
						...
						else if(dis <= windowWidth - lr && dis2 >= 0) {
							dis2 = Math.min(dis2, windowWidth - lr + 100);
							if(dis <= 0) {
								shootParticle(18 + Math.floor(Math.random() * 24), 1, lr, x + (Math.random() - 0.5) * width, Math.random() * 360, lr + (Math.random() - 0.5) * 2 * ud, x + (Math.random() - 0.5) * 2 * width, (Math.random() - 0.5) * 180);
								extra = 180 - Math.round(-dis) % 180;
			...
			for(var i = 0; i < noteRight.length; ++i) {
				...
				if(!thisNote || (isMobile && thisNote && thisNote.status != "Untouched" && !(thisNote.status == "Miss" && ! missHoldFindR[i]) && thisNote.m_type != "HOLD")) continue;
				var touchTime = thisNote.m_time * (spu);
				var dis = hiSpeed * (touchTime - thisTime);
				var x = deeMode && thisNote.m_type != "HOLD" ? mtox(thisNote.m_position, 1) * (1 - dis / (windowWidth / 2 - lr)) : mtox(thisNote.m_position, 2);
				...
				switch(thisNote.m_type) {
						...
						if (isMobile) {
							if(thisNote.status == "Untouched") {
								if (touchTime < thisTime - judge.g) {
									...
									for (var [id, f] of touchHold) {
										if (isValidTouch(f, 2) && between(f.y, mtox(thisNote.m_position, 2) - mtow(thisNote.m_width, 2)/2 - expand, mtox(thisNote.m_position, 2) + mtow(thisNote.m_width, 2)/2 + expand)) {
											if (Math.abs(thisNote.m_time*(60/barpm) - thisTime) <= judge.p) {
												hitAnime(thisNote, 2, "Perfect");
											}
											else if (Math.abs(thisNote.m_time*(60/barpm) - thisTime) <= judge.gr) {
					...
					case "HOLD":
						...
						if (isMobile) {
							...
							var miss = missHoldFindR[i];
							if (miss) {
								if (miss.half) {
									if (thisTime - miss.missTime > 0.3) {
										missHoldFindR[i] = false;
										missHoldR.delete(i);
										break;
									}
									else {
										extra = 180 - Math.round(0) % 180;
										noteHoldShow.push([2, width, dis2, x, 0, extra, 0, false, 1 - (thisTime - miss.missTime)/0.3]);
										break;
									}
								}
								else {
									if (dis < -hideHeight - (lr - hideHeight)/hide) {
										...
										extra = 180 - Math.round(-dis) % 180;
										noteHoldShow.push([2, width, dis2 - dis, x, dis, extra, 0, false, 1.0]);
										break;
									}
								}
							}
						...
						if (isMobile) {
							if (holdCheckR.has(i)) {
								...
								dis2 = Math.min(dis2, windowWidth - lr + 100);
								for (var [id, f] of touchHold) {
									if (isValidTouch(f, 2) && between(f.x, mtox(thisNote.m_position, 2) - mtow(thisNote.m_width, 2)/2 - expand, mtox(thisNote.m_position, 2) + mtow(thisNote.m_width, 2)/2 + expand)) {
										holding = true;
										break;
									}
								}
								if (holding) {
									holdCheckR.set(i, thisTime);
									extra = 180 - Math.round(-dis) % 180;
									dis = 0;
									...
									if (holdCheckR.get(i) + 0 >= noteRight[thisNote.m_subId].m_time*(60/barpm) - judge.p) {
										holdCheckR.delete(i);
										hitAnime(noteRight[thisNote.m_subId], 2, "Perfect");
									}
									else {
										noteHoldShow.push([2, width, dis2 - dis, x, dis, extra, 0, true, 1.0]);
										break;
									}
								}
								else {
									extra = 180 - Math.round(-dis) % 180;
									dis = 0;
									var releaseTime = holdCheckR.get(i);
									if (thisTime - releaseTime > holdLeaveTime) {
										if (Math.abs(releaseTime - subTime) <= judge.p) {
											hitAnime(noteRight[thisNote.m_subId], 2, "Perfect");
										}
						...
						else if(dis <= windowWidth/2 - lr && dis2 >= 0) {
							dis2 = Math.min(dis2, windowWidth/2 - lr + 100);
							if(dis <= 0) {
								shootParticle(18 + Math.floor(Math.random() * 24), 1, windowWidth - lr, x + (Math.random() - 0.5) * width, Math.random() * 360, windowWidth - lr + (Math.random() - 0.5) * 2 * ud, x + (Math.random() - 0.5) * 2 * width, (Math.random() - 0.5) * 180);
								extra = 180 - Math.round(-dis) % 180;
								dis = 0;
							}
							noteHoldShow.push([2, width, dis2 - dis, x, dis, extra, ((editSide == 0 && equal(thisNote.m_position, magPos)) ? 2 : 0) + ((editSide == 0 && equal(thisNote.m_width, magWidth)) ? 1 : 0), noteRightHit[i]]);
						}
						break;
		
					case "SUB":
						if(!noteRightHit[i] && !isMobile) {
							if(Math.abs(touchTime - thisTime) <= judge.p || touchTime <= thisTime) {
								noteRightHit[i] = true;
								hitAnime(thisNote, 2, "Perfect");
							}
